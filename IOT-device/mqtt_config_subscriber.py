...
def on_message(unused_client, unused_userdata, msg):
    ...
    with open('pub_key.pem', mode='rb') as publicfile:
        keydata = publicfile.read()
    pubkey = rsa.PublicKey.load_pkcs1(keydata)

    global configVar
    lst = ast.literal_eval(msg.payload)
    try:
        rsa.verify(lst[0], lst[1], pubkey)
        print("Verification passed.")
        maybeValidConfig = lst[0].split()
        if (maybeValidConfig[2] == datetime.datetime.now().strftime("%Y-%m-%d")):
            configVar = maybeValidConfig
            if(configVar[1] == 'True'):
                sensorAlert()
            print("configVar is now:", configVar)
